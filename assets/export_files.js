...
function buildBatchSales() {
	...
	affarr.forEach((affiliate) => {
		let newArr = [
			affiliate.Website_Id,
			"outage_estimate_" + today + "_" + affiliate.Affiliate_Id,
			"",
			Number(affiliate.salesAverage.toFixed(2)),
			"",
		];
		rows.push(newArr);
	});
	...
	link.setAttribute(
		"download",
		"Batch-Sales-Document for " + merchant.name + "_" + outage.dates
	);
