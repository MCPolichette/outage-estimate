...
function buildMerchantTable() {
	...
	document.getElementById("r_ac_commission").innerHTML =
		outage.commission_rates;
	document.getElementById("r_estimated_ac").innerHTML = toUSD(
		totals.estimatedComm
	);
	document.getElementById("r_estimated_nc").innerHTML = toUSD(
		totals.networkComm
	);
	document.getElementById("r_estimated_total").innerHTML = toUSD(
		totals.networkComm + totals.estimatedComm
	);
	document.getElementById("r_a_count").innerHTML =
		merchant.affiliate_count +
		" Affiliates were selected based on their click volume during the outage.  This accounts for " +
		click_percent +
		" of the total click volume during this time.";
	if (document.getElementById("report_notes").value) {
		document.getElementById("r_note1").innerHTML =
...
function buildOutageTable() {
	...
	outage.dates =
		DateToString(outage.date_start_display) +
		" to " +
		DateToString(outage.date_end_display);
	table.innerHTML = "";
	...
	build2columns(
		...
		4,
		"Conversion Rate :",
		(baseline.conversion_rate * 100).toFixed(2) + " %"
	);
	...
	build2columns(
		table,
		12,
		"Adjusted Discrepency :",
		toUSD(outage.adjusted_discrepency)
	);
...
function buildAffiliateTable() {
	...
	var arrheader = [
		...
		"Affiliate Name",
		"Click Throughs",
		"Percentage based on Clicks",
		"Tracked Sales during the outage",
		"Estimated Sales by AOV, and Conversion",
		"Estimated Sales by Click Percentage",
		"Average of Two Sales Figures",
		"Estimated Affiliate Commission",
		"Estimated Network Commission",
	];
	...
	totals.percent_of_total_clicks =
		((totals.clicks / outage.total_clicks) * 100).toFixed(2) + "%";
	...
	for (var i = 0; i < affarr.length; i++) {
		...
		let percentageBasedOnClicks = Number(
			(affarr[i].clicks / totals.clicks).toFixed(4)
		);
		let estimatedSalesBasedOnPercent = Number(
			(percentageBasedOnClicks * outage.discrepency).toFixed(2)
		);
		affarr[i].salesAverage =
			(affarr[i].estimatedAOV + estimatedSalesBasedOnPercent) / 2;
		let estimatedCommission = Number(
			(affarr[i].salesAverage * affarr[i].commissionRate).toFixed(2)
		);
		...
		totals.estimatedSalesbyAOV =
			totals.estimatedSalesbyAOV + affarr[i].estimatedAOV;
		totals.estimatedSalesByClick =
			totals.estimatedSalesByClick + estimatedSalesBasedOnPercent;
	...
	let smallest_commission_rate = findMin(
		totals.commission_rates,
		totals.commission_rates.length
	);
	let largest_commission_rate = findMax(
		totals.commission_rates,
		totals.commission_rates.length
	);
	if (smallest_commission_rate === largest_commission_rate) {
		outage.commission_rates =
			(smallest_commission_rate * 100).toFixed(2) + "%";
	} else {
		outage.commission_rates =
			(smallest_commission_rate * 100).toFixed(2) +
			"% - " +
			(largest_commission_rate * 100).toFixed(2) +
			"%";
	}
	...
	let t1 = document.createTextNode(
		"Totals from the top " + merchant.affiliate_count + " Affiliates"
	);
	c1.scope = "row";
	c1.colSpan = "2";
	c3.colSpan = "2";
...
function table_border(table_id) {
	...
	for (i = 0; i < totalRowCount; i++) {
		table.rows[i].cells[0].style.cssText +=
			"border-left-width :5px;  border-style:solid; border-left-color: #cfe2ff";
		table.rows[i].cells[9].style.cssText +=
